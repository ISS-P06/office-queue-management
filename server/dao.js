...
export function maxTicketNum() {
  let today = dayjs().format('YYYY-MM-DD');
  return new Promise((resolve, reject) => {
    let sql = 'SELECT MAX(number) AS lastNumber FROM Ticket where date = ?';
    db.get(sql, [today], (err, row) => {
      if (err) reject(err);
      else resolve(row.lastNumber);
    });
  });
}

export async function insertNewTicket(serviceID) {
  let maxTicketNumber = await maxTicketNum();
  let ticketNumber = maxTicketNumber + 1;
  let today = dayjs().format('YYYY-MM-DD');

  return new Promise((resolve, reject) => {
    const sql = 'INSERT INTO Ticket(ref_service, date, status, number) VALUES (?,?,?,?)';
    db.run(sql, [serviceID, today, 'in-queue', ticketNumber], (err) => {
      if (err) {
        console.log(err);
        reject(err);
        return;
